...
export function CampaignActions({ previewPath, isSaved, onSave, onPublish }: CampaignActionsProps) {
  ...
  return (
    <section className="flex flex-col gap-3 rounded-2xl border border-white/70 bg-white/80 p-4 shadow-[0_12px_36px_-24px_rgba(16,35,63,0.45)] backdrop-blur-md sm:flex-row dark:border-white/10 dark:bg-[#11151c]/75">
      <Button variant="secondary" className="h-11 w-full justify-center gap-2" onClick={onSave}>
        <Save size={15} />
        {isSaved ? 'Salvar Alteracoes' : 'Salvar'}
      </Button>

      <Button className="h-11 w-full justify-center gap-2" onClick={onPublish}>
        <Send size={15} />
        Publicar
      </Button>
