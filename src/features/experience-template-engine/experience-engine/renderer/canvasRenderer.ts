...
import type { ExperienceTemplateConfig, LayoutRegion, RenderWarning } from '../core/types'
import {
  getColor,
  getRegionPadding,
  getThemePalette,
...
function toBox(
  ...
  const x = scaler.horizontalScale(region.x * template.width) + margin + padding
  const y = scaler.verticalScale(region.y * template.height) + margin + padding
  const width = Math.max(1, scaler.horizontalScale(region.width * template.width) - margin * 2 - padding * 2)
  const height = Math.max(1, scaler.verticalScale(region.height * template.height) - margin * 2 - padding * 2)
...
export function renderTemplateCanvas(
  ...
  template.nodes.forEach((node) => {
    const region = template.layout[node.region]
    if (!region) return

    const box = toBox(region, template, canvasPreset.width, canvasPreset.height)
